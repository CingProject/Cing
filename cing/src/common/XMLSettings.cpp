...
namespace Cing
{
    ...
    // Creates an empty the xml file
    void XMLSettings::init( const std::string& topLevelName )
    {
        m_xmlFile.init( topLevelName );
        m_valid = true;
    }
    ...
    bool XMLSettings::save( const std::string& xmlFileName )
    {
        ...
        std::string fullPath = xmlFileName;
        if ( isPathAbsolute( fullPath ) == false )
        {
            fullPath = dataFolder + fullPath;
        }
