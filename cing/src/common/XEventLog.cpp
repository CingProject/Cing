								...
								LPCTSTR lpszEventMessageDll)
{
	...
	//_tcsncpy(szKey, szRegPath, _MAX_PATH*2);
	_tcsncpy_s(szKey, _MAX_PATH*2, szRegPath, strlen(szRegPath));
	//_tcscat(szKey, lpszApp);
	_tcscat_s(szKey, _MAX_PATH*2, lpszApp);
	TRACE(_T("szKey=<%s>\n"), szKey);
	...
	if (lRet == ERROR_SUCCESS)
	{
		...
		if (lpszEventMessageDll)
		{
			...
			_tcsncpy_s(szPathName, _MAX_PATH*2-2, lpszEventMessageDll, strlen(lpszEventMessageDll)); // _MAX_PATH*2-2);
		}
		else
		{
			...
			_tcscat_s(szPathName, _MAX_PATH*2-2, _T("\\XEventMessage.dll"));
		}
...
void CXEventLog::SetAppName(LPCTSTR lpszApp)
{
	...
	if (m_pszAppName)
	{
		...
		_tcsncpy_s(m_pszAppName, _MAX_PATH*2-2, lpszApp, strlen(lpszApp));
	}
}
