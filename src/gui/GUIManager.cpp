...
bool GUIManager::init()
{
  ...
  m_quickGUIManager = m_quickGuiRoot->createGUIManager( activeCamera.getOgreCamera()->getViewport() , "qgui" );
  m_quickGUIManager->setSceneManager( &(Graphics::GraphicsManager::getSingleton().getSceneManager()) );

  // Store the default sheet pointer (this sheet will used to place default widgets on it)
  m_defaultSheet = m_quickGUIManager->getDefaultSheet();
  ...
  // Hide mouse
  m_quickGUIManager->getMouseCursor()->hide();
 ...
 */
void GUIManager::setDebugText( const char* text )
{
