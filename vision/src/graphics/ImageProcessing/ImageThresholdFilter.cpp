 ...
 */
void ImageThresholdFilter::apply( const IplImage& inImage )
{
  // Check valid filter
  if ( !isValid() )
  ...
  if( (inImage.imageSize != m_outputImage->imageSize) )
  ...
  cvThreshold( &inImage, m_outputImage, m_threshold, 255, CV_THRESH_BINARY );
}
