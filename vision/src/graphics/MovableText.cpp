...
namespace Graphics
{
    ...
    void MovableText::checkMemoryAllocation( size_t numChars )
    {
        ...
        if( mAllocSize < numChars)
        {
            ...
            // positions & texcoords
            Ogre::HardwareVertexBufferSharedPtr vbuf = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(	decl->getVertexSize(POS_TEX_BINDING), 
	...
	void MovableText::_setupGeometry()
	{
		...
		if ( mVerticalAlignment != TOP )
		{
			...
			for (i = mCaption.begin(); i != iend; ++i)
			{
				...
				if ( IsNewLine(i.getCharacter()) || isLineBreak )
	...
	float MovableText::getWordLength( Ogre::DisplayString::iterator it, Ogre::DisplayString::iterator& end )
	{
		float length = 0;
		while( !IsNewLine( it.getCharacter() ) && !IsSpace( it.getCharacter() ) && (it != end) )
		{
			length += mpFont->getGlyphAspectRatio(it.getCharacter()) * mCharHeight;
			++it;
		}
	...
	void MovableText::advanceWord( Ogre::DisplayString::iterator& it, Ogre::DisplayString::iterator& end  )
	{
		while( !IsNewLine( (it+1).getCharacter() ) && !IsSpace( (it+1).getCharacter() ) && ((it+1) != end) )
			++it;
	}

	void MovableText::calculateLineLeft( const Ogre::DisplayString::iterator& currentIt, const Ogre::DisplayString::iterator& iend, const ForcedLineBreaks& forcedLineBreaks, float& left, float& lineLength )
	{
		...
		if ( mHorizontalAlignment != LEFT )
		{
			for( Ogre::DisplayString::iterator j = currentIt; j != iend; j++ )
			{
				...
				if (IsNewLine(character) || (std::find(forcedLineBreaks.begin(), forcedLineBreaks.end(), index) != forcedLineBreaks.end() ) ) 
				{
					break;
				}
				else if (IsSpace(character)) // space
	...
	void MovableText::drawCharacter( float*& pVert, const Ogre::Font::CodePoint& character, float& top, float& left, float& lineLength )
	{
		Vector currPos;
